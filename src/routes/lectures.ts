...
router.get("/teacher/:teacherId", async (req, res) => {
  try {
    ...
    const teacherClasses = await db
      ...
      .from(classes)
      .where(eq(classes.teacherId, teacherId as string));
    ...
    const filterConditions = [inArray(lectures.classId, teacherClassIds)];
...
router.get("/student/:studentId", async (req, res) => {
  try {
    ...
    // Build filter conditions
    const filterConditions = [inArray(lectures.classId, enrolledClassIds)];
  ...
  try {
    ...
    if (req.user?.role === "student") {
      const [enrollment] = await db
        ...
        .where(
          and(
            eq(enrollments.classId, lecture.classId),
            eq(enrollments.studentId, req.user.id),
          ),
        );
