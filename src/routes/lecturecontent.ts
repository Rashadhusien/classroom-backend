...
router.get("/", async (req, res) => {
  try {
    ...
    const [lectureRecord] = await db
      .select({ id: lectures.id, classId: lectures.classId })
      .from(lectures)
    ...
    if (req.user?.role === "student") {
      const [enrollment] = await db
        ...
        .where(
          and(
            eq(enrollments.classId, lectureRecord.classId),
...
router.get("/:id", async (req, res) => {
  try {
    ...
    // Check enrollment for students
    if (req.user?.role === "student") {
      // Get the lecture to find the class
      const [lectureRecord] = await db
        .select({ classId: lectures.classId })
        .from(lectures)
        .where(eq(lectures.id, content.lectureId));
